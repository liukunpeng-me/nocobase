...
// v2 only — see AGENTS.md §Project Structure.
...
/**
 * v2 mirror of useTTSEnabled. Persistence uses v2's `useApp().apiClient`
 * rather than v1's useAPIClient hook.
 */
export function useTTSEnabled(): UseTTSEnabledResult {
  ...
  const persist = useCallback(
    async (next: boolean) => {
      ...
      await apiClient.resource('aiEmployees').updateUserVoiceOverride({
        values: {
    ...
    [apiClient, currentEmployee, voiceOverride, setCurrentEmployee],
  );
